...
import {syncAppendToFile} from "@lib/base/dist";
...
const exec = async () => {
    ...
    fs.readdir(dirpath, function (err, files) {
        ...
        txtFiles.forEach(file0 => {
            const freeRAM = os.freemem();
            if (freeRAM / (1024 * 1024) < 1024) {
                syncAppendToFile(path_ + 'output-' + counter + '.csv', row_prepared);
                row_prepared = '';
                counter += 1;
            }

            const file = path_ + file0;
            ...
            if (file.indexOf('2022.zip') > -1) { // вставка сначала только 2022 года
            ...
            if (fileSizeInBytes > 0) {
                ...
                zipEntries.forEach(function (zipEntry) {
                    ...
                    rows_splitted.forEach(function (row) {
                        ...
                        const time = getUnixTime(new Date(row_split[1]));
...
*/
                ...
                //console.log('mem='+info.freeMemMb)
        ...
        //counter += 1;
        syncAppendToFile(path_ + 'output-another.csv', row_prepared);

        console.log(empty_zip_files)

            // next cExec type file .....
        // дозагрузка свечей
            // type output-another.csv | docker run -i --rm --link clickhouse_host:clickhouse-server clickhouse/clickhouse-client:21.3.20.1 --host clickhouse-server --format_csv_delimiter=";" --query="INSERT INTO default.GetCandles FORMAT CSV"
